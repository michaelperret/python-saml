...
%(org)s
        ...
        if not isinstance(sp['NameIDFormat'], list):
            sp['NameIDFormat'] = [sp['NameIDFormat']]
...
%(organization)s%(contacts)s</md:EntityDescriptor>""" % \
            {
                ...
                'wsign': str_wsign,
                'name_id_format_string': ('        <md:NameIDFormat>{}</md:NameIDFormat>\n'*len(sp['NameIDFormat'])).format(*sp['NameIDFormat']),
